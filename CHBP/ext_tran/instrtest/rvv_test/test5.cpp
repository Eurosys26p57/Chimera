...
// 矩阵乘法（使用RVV向量扩展）
void matrix_mul_vector_rvv(int32** A, int32** B, int32** C, int N) {
...
// 矩阵乘法（无向量扩展版本）
void matrix_mul_vector_plain(int32** A, int32** B, int32** C, int N) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            C[i][j] = 0;
            for (int k = 0; k < N; k++) {
        ...
        threads.emplace_back([i, core_id, &total_fib_time, &total_matrix_time, half_cores, experiment, matrix_ratio, &dis_ratio, N, fib_n]() {
            ...
            if (experiment == 1) {
                ...
                if (r < matrix_ratio) {
                    ...
                    matrix_mul_vector_plain(A, B, C, N);
                    for (int x = 0; x < N; ++x) {
