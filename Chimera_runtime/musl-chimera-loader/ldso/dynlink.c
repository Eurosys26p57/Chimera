...
static int32_t section_changed = 0;
#define PAGE_SIZE 4096
#define PAGE_ROUND_DOWN(p) ((off_t)((p) & ~(PAGE_SIZE - 1)))
#define PAGE_ROUND_UP(p)   ((off_t)(((p) + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1)))


void copy_sections(Elf64_Ehdr *eh, int fd,struct dso *dso, const char *SECTION_NAME_SRC, const char *SECTION_NAME_DST) {
    if (SECTION_NAME_SRC && SECTION_NAME_DST && !section_changed) {
        ssize_t l;
        size_t i;
        ...
        l = pread(fd, shdr, eh->e_shnum * sizeof(Elf64_Shdr), eh->e_shoff);
        if (l == -1) {
            perror("pread");
            free(shdr);
            return;
        ...
        char *shstrtab = malloc(shdr[eh->e_shstrndx].sh_size);
        if (shstrtab == NULL) {
            perror("malloc");
            free(shdr);
            return;
        ...
        l = pread(fd, shstrtab, shdr[eh->e_shstrndx].sh_size, shdr[eh->e_shstrndx].sh_offset);
        if (l == -1) {
            perror("pread");
            free(shdr);
            free(shstrtab);
		...
		Elf64_Phdr *phdr = malloc(eh->e_phnum * sizeof(Elf64_Phdr));
		if (!phdr) {
			perror("malloc phdr");
			free(shdr);
			free(shstrtab);
		...
		l = pread(fd, phdr, eh->e_phnum * sizeof(Elf64_Phdr), eh->e_phoff);
		if (l == -1) {
			perror("pread phdr");
			free(shdr);
			free(shstrtab);
        ...
        Elf64_Shdr *src_section = NULL;
        Elf64_Shdr *dst_section = NULL;

        for (i = 0; i < eh->e_shnum; i++) {
            if (strcmp(shstrtab + shdr[i].sh_name, SECTION_NAME_SRC) == 0) {
                src_section = &shdr[i];
            } else if (strcmp(shstrtab + shdr[i].sh_name, SECTION_NAME_DST) == 0) {
                dst_section = &shdr[i];
            }
        }
		...
		if (src_section && dst_section){
			...
			for (size_t j = 0; j < eh->e_phnum; j++){
			    Elf64_Phdr *seg = &phdr[j];
    			if (seg->p_type != PT_LOAD) continue;
			
			    if (src_section->sh_offset >= seg->p_vaddr &&
					src_section->sh_offset < seg->p_vaddr + seg->p_memsz) {
					size_t offset = src_section->sh_offset - seg->p_vaddr;
					src_addr = (size_t)dso -> base + seg->p_vaddr + offset;
				}

				if (dst_section->sh_offset >= seg->p_vaddr &&
					dst_section->sh_offset < seg->p_vaddr + seg->p_memsz) {
					size_t offset = dst_section->sh_offset - seg->p_vaddr;
					dst_addr = (size_t)dso -> base + seg->p_vaddr + offset;
				}

			}

			char *src_start_page = (char*)PAGE_ROUND_DOWN(src_addr);
            char *src_stop_page  = (char*)PAGE_ROUND_UP(src_addr + src_section->sh_size);
            char *dst_start_page = (char*)PAGE_ROUND_DOWN(dst_addr);
            char *dst_stop_page  = (char*)PAGE_ROUND_UP(dst_addr + dst_section->sh_size);

            if (mprotect(src_start_page, src_stop_page - src_start_page, PROT_READ) < 0) {
                perror("mprotect src");
                free(shdr);
                free(shstrtab);
                return;
            }
            if (mprotect(dst_start_page, dst_stop_page - dst_start_page, PROT_READ | PROT_WRITE) < 0) {
                perror("mprotect dst");
                free(shdr);
                free(shstrtab);
                return;
			...
			void *src_ptr = (void *)src_addr;
            void *dst_ptr = (void *)dst_addr;

            size_t copy_size = src_section->sh_size ;
            memcpy(dst_ptr, src_ptr, copy_size);

            void *end_ptr = (char *)dst_ptr + copy_size;
            size_t remain = dst_section->sh_size - copy_size;
            if (remain > 0) {
                memset(end_ptr, 0xcc, remain);
            }

            if (mprotect(src_start_page, src_stop_page - src_start_page, PROT_READ) < 0) {
                perror("mprotect src restore");
            }
            if (mprotect(dst_start_page, dst_stop_page - dst_start_page, PROT_READ | PROT_EXEC) < 0) {
                perror("mprotect dst restore");
            }
...
done_mapping:
	...
	char *SECTION_NAME_SRC = getenv("SECTION_SRC");
    char *SECTION_NAME_DST = getenv("SECTION_DST");
	copy_sections(eh, fd, dso, SECTION_NAME_SRC, SECTION_NAME_DST);
